...
class CircularProgressBar: UIView {
    ...
    public var labelSize: CGFloat = 14 {
        didSet {
    ...
    private func drawForegroundLayer(){
        ...
        //foregroundLayer.strokeEnd = 0 //This Key set progress to 0 again.
    ...
    private func makeLabel(withText text: String) -> UILabel {
        ...
        label.font = UIFont.systemFont(ofSize: labelSize)
        label.sizeToFit()
        label.center = CGPoint(x: pathCenter.x, y: pathCenter.y - 38)
        return label
    }
    
    private func makeImage() -> UIImageView {
        let imgView = UIImageView(frame: CGRect(x: 0, y: 0, width: 30, height: 30))
        imgView.image = #imageLiteral(resourceName: "icn_run")
        imgView.center = pathCenter
        return imgView
    }
    ...
    private func setupView() {
        makeBar()
        label = makeLabel(withText: "Today")
        imgView = makeImage()
